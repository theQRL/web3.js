...
helpFunction() {
	echo "Usage: $0 <geth | testnet | mainnet> <http | ws> [node | electron | firefox | chrome] [coverage | sync]"
...
SUPPORTED_BACKENDS=("geth" "testnet" "mainnet")
...
if [[ $BACKEND == "geth" || $BACKEND == "geth-binary" ]]; then
