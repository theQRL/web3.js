...
describeIf(isSocket)('ens events', () => {
	...
	beforeAll(async () => {
		...
		const Registry = new web3.zond.Contract(ENSRegistryAbi, undefined, {
			provider: getSystemTestProvider(),
		});

		const NameWrapper = new web3.zond.Contract(NameWrapperAbi, undefined, {
			provider: getSystemTestProvider(),
		});

		Resolver = new web3.zond.Contract(PublicResolverAbi, undefined, {
			provider: getSystemTestProvider(),
		});
